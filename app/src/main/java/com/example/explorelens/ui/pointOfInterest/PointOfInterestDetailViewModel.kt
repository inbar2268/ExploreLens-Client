...
class PointOfInterestDetailViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val nearbyPlacesRepository = PointOfInterestRepository.getInstance(application)
...
// Replace your placeDetailState LiveData with this:
    ...
    fun setPlaceId(placeId: String) {
        Log.d("PointOfInterestDetailViewModel", "setPlaceId called with: $placeId")

        if (_placeId.value != placeId) {
            _placeId.value = placeId
            Log.d("PointOfInterestDetailViewModel", "PlaceId set to: $placeId")

            // Debug: Test mock place creation
            if (placeId.startsWith("mock_")) {
                val mockPlace = createMockPlace(placeId)
                Log.d("PointOfInterestDetailViewModel", "Mock place created: ${mockPlace?.name ?: "null"}")
            }
        } else {
            Log.d("PointOfInterestDetailViewModel", "PlaceId unchanged: $placeId")
        }
    }
