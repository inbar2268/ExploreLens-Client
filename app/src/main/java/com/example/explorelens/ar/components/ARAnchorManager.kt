...
import com.example.explorelens.data.repository.ARAnchorRepository
...
public class ARAnchorManager(
    ...
    companion object {
        private const val TAG = "ARAnchorManager"
    }
    ...
    private fun getAllAnchors() {
        ARAnchorRepository.shared.getAlArLabeledAnchors { fetchedAnchors ->
            synchronized(arLabeledAnchors) {
                arLabeledAnchors.clear()
    ...
    private fun addAnchorToDatabase(anchor: ARLabeledAnchor) {
        ARAnchorRepository.shared.addArLabelAnchor(anchor) {
            getAllAnchors()
        }
    }
}