...
import com.example.explorelens.data.model.pointOfInterest.GeoLocation
import com.example.explorelens.data.model.pointOfInterest.OpeningHours
import com.example.explorelens.data.model.pointOfInterest.PointOfInterest
import com.example.explorelens.data.repository.PointOfInterestRepository
import com.example.explorelens.ui.pointOfInterest.PointOfInterestDetailFragment
...
class PointOfInterestAnchorManager(
    ...
    companion object {
        private const val TAG = "PointOfInterestAnchorManager"
    ...
    private val nearbyPlacesRepository = PointOfInterestRepository.getInstance(activity)
    ...
    private fun showLayerDetailAsFullScreen(placeId: String) {
        try {
            activity.runOnUiThread {
                val fragment = PointOfInterestDetailFragment.newInstance(placeId)
                ...
                activity.supportFragmentManager.beginTransaction()
                    .add(android.R.id.content, fragment, "PointOfInterestDetailFragment")
                    .addToBackStack("LayerDetail")
                    .commitAllowingStateLoss()

                Log.d(TAG, "Added PointOfInterestDetailFragment directly to content for placeId: $placeId")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error showing PointOfInterestDetailFragment", e)
