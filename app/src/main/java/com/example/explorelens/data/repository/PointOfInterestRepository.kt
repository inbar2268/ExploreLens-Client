...
class PointOfInterestRepository(private val context: Context) {

    companion object {
        private const val TAG = "PointOfInterestRepository"
        private const val CACHE_TIMEOUT_HOURS = 24L

        @Volatile
        private var INSTANCE: PointOfInterestRepository? = null

        fun getInstance(context: Context): PointOfInterestRepository {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: PointOfInterestRepository(context.applicationContext).also { INSTANCE = it }
            }
        }
    }
    ...
    /**
     * Get individual place details as LiveData for PointOfInterestDetailFragment
     */
